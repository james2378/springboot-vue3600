		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','gender') || $check_field('add','gender') || $check_field('set','gender')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="性别" prop="gender">
					<el-select id="gender" v-model="form_sub['gender']"
						v-if="user_group === '管理员' || (form_sub['doctor_id'] && $check_field('set','gender')) || (!form_sub['doctor_id'] && $check_field('add','gender'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','position') || $check_field('add','position') || $check_field('set','position')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="职位" prop="position">
					<el-input id="position" v-model="form_sub['position']" placeholder="请输入职位"
							  v-if="user_group === '管理员' || (form_sub['doctor_id'] && $check_field('set','position')) || (!form_sub['doctor_id'] && $check_field('add','position'))"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','department') || $check_field('add','department') || $check_field('set','department')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="科室" prop="department">
					<el-input id="department" v-model="form_sub['department']" placeholder="请输入科室"
							  v-if="user_group === '管理员' || (form_sub['doctor_id'] && $check_field('set','department')) || (!form_sub['doctor_id'] && $check_field('add','department'))"></el-input>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 提交附加信息
			submit_sub(form_sub){
				if(form_sub["doctor_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?doctor_id=" + form_sub["doctor_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/doctor/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/doctor/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/doctor/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/doctor/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/doctor/view','get');
