<template>
	<div class="diy_details_box diy_div_patient_medical_record">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "patient_medical_record",
					source_field: "patient_medical_record_id",
					source_id: this.obj["patient_medical_record_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/patient_medical_record/set?patient_medical_record_id=' + _this.obj["patient_medical_record_id"], {
								praise_len
							}, (res) => {
								if(res.result){
