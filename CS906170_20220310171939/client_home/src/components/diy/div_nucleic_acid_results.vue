<template>
	<div class="diy_details_box diy_div_nucleic_acid_results">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "nucleic_acid_results",
					source_field: "nucleic_acid_results_id",
					source_id: this.obj["nucleic_acid_results_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/nucleic_acid_results/set?nucleic_acid_results_id=' + _this.obj["nucleic_acid_results_id"], {
								praise_len
							}, (res) => {
								if(res.result){
