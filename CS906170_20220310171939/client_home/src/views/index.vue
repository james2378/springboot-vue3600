		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/department_information/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_department_information = !show_list_department_information">
		                        <div class="switch_box">
		                            <span v-if="show_list_department_information"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--科室信息推荐列表组件 -->
		                <list_department_information v-if="show_list_department_information" :list="list_department_information" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_department_information.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_department_information" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_department_information" :key="i" @click="to_details('department_information',o,'department_information_id')">
		                            <td v-for="(oj,n) in fields_department_information">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
...
<script>
	...
	import list_department_information from "@/components/diy/list_department_information.vue";
	...
	export default {
		...
		data() {
			return {
				...
				show_list_department_information: true,
				list_department_information: [],
				vm_arr_department_information: [
		...
		methods: {
			...
			get_department_information() {
				let url = "~/api/department_information/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 8
				}, (json) => {
					if (json.result) {
						this.list_department_information = json.result.list;
					}
				})
			},
		...
		mounted() {
			this.get_department_information();
			this.get_menu();
			this.get_slides();
			this.get_article();
			this.get_link();
		...
		computed: {
		    list_table_department_information() {
		        var list = this.list_department_information;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_department_information.map((o) => {
		                // 第二个中括号是对象的属性
